...
int8_t
insert_at_Vector(Vector * vec, void *obj, u_int32_t loc)
{
  size_t offset;
...
void *
return_at_Vector(Vector * vec, u_int32_t loc)
{
  register u_int32_t offset = (loc * O(vec));
  CHECK_VARN(vec, NULL);
  if(loc > C(vec)) {
	  return NULL;
  }
